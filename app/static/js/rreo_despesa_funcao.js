// JavaScript para RREO - Demonstrativo de Despesas por Função

console.log('Arquivo rreo_despesa_funcao.js carregado');
...
$(document).ready(function() {
    console.log('Document ready - iniciando RREO Despesa por Função');
    carregarFiltros();
    configurarEventos();
});
...
function carregarFiltros() {
    ...
    $.ajax({
        url: '/rreo-despesa-funcao/api/filtros',
        method: 'GET',
...
function gerarRelatorio() {
    ...
    console.log('📊 Gerando relatório RREO Despesa por Função:', {ano, bimestre});
    ...
    $.ajax({
        url: '/rreo-despesa-funcao/api/gerar-relatorio',
        method: 'GET',
        data: {
            ano: ano,
            bimestre: bimestre
...
function construirTabela(dados) {
    ...
    if (dados.despesas_exceto_intra.total.dotacao_inicial > 0 ||
        dados.despesas_exceto_intra.total.dotacao_autorizada > 0 ||
        dados.despesas_exceto_intra.total.empenhado_ate_bimestre > 0) {
        ...
        // Adicionar funções e subfunções
        construirFuncoesSubfuncoes(tbody, dados.despesas_exceto_intra.funcoes);
    }
    ...
    if (dados.despesas_intra.total.dotacao_inicial > 0 ||
        ...
        // Adicionar funções e subfunções
        construirFuncoesSubfuncoes(tbody, dados.despesas_intra.funcoes);
    }
...
// Construir funções e subfunções
function construirFuncoesSubfuncoes(tbody, funcoes) {
    // Ordenar as funções por código
    const funcoesOrdenadas = Object.keys(funcoes).sort((a, b) => {
        const numA = parseInt(a) || 999;
        const numB = parseInt(b) || 999;
        return numA - numB;
    });
    
    funcoesOrdenadas.forEach(function(codigoFuncao) {
        const funcao = funcoes[codigoFuncao];
        
        // Adicionar linha da função (título do grupo)
        tbody.append(criarLinhaGrupo(
            funcao.nome.toUpperCase(),
            funcao.total
        ));
        
        // Ordenar subfunções por código
        const subfuncoesOrdenadas = Object.keys(funcao.subfuncoes).sort((a, b) => {
            const numA = parseInt(a) || 999;
            const numB = parseInt(b) || 999;
            return numA - numB;
        });
        
        // Adicionar subfunções
        subfuncoesOrdenadas.forEach(function(codigoSubfuncao) {
            const subfuncao = funcao.subfuncoes[codigoSubfuncao];
            tbody.append(criarLinhaCategoria(subfuncao.nome, subfuncao));
        });
    });
}
...
function limparFiltros() {
    ...
    $('#mensagemInicial').removeClass('alert-danger').addClass('alert-info');
    $('#mensagemInicial').html(
        '<i class="bi bi-info-circle"></i> Selecione os filtros acima para gerar o relatório RREO de Despesas por Função'
    );
    $('#mensagemInicial').show();
...
function exportarExcel() {
    ...
    csv.push(['DEMONSTRATIVO DE DESPESAS POR FUNÇÃO/SUBFUNÇÃO']);
    ...
    if (dados.despesas_exceto_intra.total.dotacao_inicial > 0 ||
        ...
        // Ordenar e adicionar funções
        const funcoesOrdenadas = Object.keys(dados.despesas_exceto_intra.funcoes).sort((a, b) => {
            const numA = parseInt(a) || 999;
            const numB = parseInt(b) || 999;
            return numA - numB;
        });
        ...
        funcoesOrdenadas.forEach(function(codigoFuncao) {
            const funcao = dados.despesas_exceto_intra.funcoes[codigoFuncao];
            adicionarLinhaCSV(funcao.nome.toUpperCase(), funcao.total, 1);
            
            // Ordenar e adicionar subfunções
            const subfuncoesOrdenadas = Object.keys(funcao.subfuncoes).sort((a, b) => {
                const numA = parseInt(a) || 999;
                const numB = parseInt(b) || 999;
                return numA - numB;
            ...
            subfuncoesOrdenadas.forEach(function(codigoSubfuncao) {
                const subfuncao = funcao.subfuncoes[codigoSubfuncao];
                adicionarLinhaCSV(subfuncao.nome, subfuncao, 2);
            });
        });
    }
    ...
    if (dados.despesas_intra.total.dotacao_inicial > 0 ||
        ...
        // Ordenar e adicionar funções
        const funcoesOrdenadas = Object.keys(dados.despesas_intra.funcoes).sort((a, b) => {
            const numA = parseInt(a) || 999;
            const numB = parseInt(b) || 999;
            return numA - numB;
        });
        ...
        funcoesOrdenadas.forEach(function(codigoFuncao) {
            const funcao = dados.despesas_intra.funcoes[codigoFuncao];
            adicionarLinhaCSV(funcao.nome.toUpperCase(), funcao.total, 1);
    ...
    let filename = `RREO_Despesa_Funcao_${dadosRelatorio.ano}_Bim${dadosRelatorio.bimestre}.csv`;
