"""
Script para carga incremental de ReceitaLancamento
"""
import sys
import os
...
def analisar_arquivo_periodo(arquivo):
    ...
    try:
        ...
        df = pd.read_excel(arquivo, nrows=5000)  # Ler mais linhas para garantir variedade
        ...
        # Para arquivos grandes, estimar baseado no tamanho do arquivo
...
def main():
    ...
    print("=" * 80)
    print("ETL - RECEITA LANÇAMENTO (CARGA INCREMENTAL)")
    print("=" * 80)
    
    # Verificar argumento do arquivo
    if len(sys.argv) < 2:
        arquivo = "dados_brutos/fato/ReceitaLancamento.xlsx"
        print(f"\n⚠️  Nenhum arquivo especificado.")
        print(f"Usando arquivo padrão: {arquivo}")
        print("\n💡 Dica: Use 'python scripts/load_receita_lancamento_incremental.py ReceitaLancamentoJulho.xlsx'")
    else:
    ...
    if total_estimado > 100000:
        print(f"\n⚠️  ATENÇÃO: Arquivo grande! Tempo estimado: {total_estimado//50000 * 2}-{total_estimado//50000 * 3} minutos")
            ...
            if periodos_processados:
                ...
                for periodo in sorted(periodos_processados):
                    ...
                    if result:
                        ...
                        print(f"      - Diferença: R$ {row[3] - row[4]:,.2f}")
        except Exception as e:
